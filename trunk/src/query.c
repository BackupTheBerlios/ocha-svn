        ...
        if(strcmp(name_prepared, query_prepared)==0) {
                ...
                do {
                        ...
                        if(found==NULL) {
                                ...
                                int start_index = g_utf8_pointer_to_offset(name_prepared,
                                                                           found);
                                int end_index = start_index+g_utf8_strlen(current,
                                                                          -1);
                                const char *start_ptr = g_utf8_offset_to_pointer(name,
                                                                                 start_index);
                                const char *end_ptr = g_utf8_offset_to_pointer(name,
                                                                               end_index);

                                char *highlight_ptr=highlight+(start_ptr-name);
                                for(const char *cur=start_ptr;
                                    cur<end_ptr;
                                    cur++, highlight_ptr++) {
                                        *highlight_ptr=*cur;
                                }
