...
static void resultholder_refresh(const char *query, struct resultholder *, float pertinence);
...
void resultlist_executed(struct result *result)
{
    ...
    struct resultholder *holder = (struct resultholder *)g_hash_table_lookup(hash, result->path);
    if(holder)
